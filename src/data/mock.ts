...
export type RequestComment = {
  ...
  date?: string;
  // Сообщение было отредактировано автором (показываем пометку "изменено",
  // как в Телеграме).
  edited?: boolean;
  // Только на клиенте: сообщение показано оптимистично и ещё не подтверждено
  // сервером (id временный) — его нельзя редактировать/удалять/помечать
  // прочитанным.
  pending?: boolean;
};
