    ...
    <AppShell>
      <div className="bg-background pt-5 pb-3 desktop:sticky desktop:top-0 desktop:z-20 desktop:border-b desktop:border-border desktop:pt-6 desktop:shadow-[0_8px_12px_-10px_rgba(15,23,42,0.35)] xl:pt-8">
        ...
        <div className="mt-4 flex items-center gap-3 desktop:hidden">
          <Link
            to="/records/new"
            className="flex-1 rounded-xl bg-primary px-5 py-3 text-center text-sm font-semibold text-primary-foreground shadow-[0_10px_20px_-6px_rgba(15,23,42,0.45)] transition-all duration-200 hover:-translate-y-0.5 hover:scale-105 hover:shadow-[0_16px_28px_-6px_rgba(15,23,42,0.55)] active:translate-y-0 active:scale-100"
          >
            Новая запись
          </Link>
          <button
            onClick={() => setFiltersOpen((v) => !v)}
            className="relative flex shrink-0 items-center gap-2 rounded-xl border border-border bg-surface px-4 py-3 text-sm font-semibold"
          >
          ...
          {hasActiveFilters && (
            <button
              onClick={clearFilters}
              className="flex shrink-0 items-center gap-1.5 rounded-lg border border-border px-2.5 py-2 text-xs font-semibold text-muted-foreground transition-colors hover:bg-muted hover:text-foreground"
            >
              <X className="size-3.5" />
              Очистить
            </button>
              ...
              >
                <span className="block">
                  <span className="block break-words whitespace-normal">
                    {object ? (
                      <>
                        <span className="inline-flex items-center rounded-full bg-orange-100 px-2.5 py-1 text-sm font-semibold text-orange-800">
                          {object.name}
                        </span>{" "}
                        {object.address && (
