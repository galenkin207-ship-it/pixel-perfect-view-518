...
self.addEventListener("push", (event) => {
  ...
  const options = {
    ...
    icon: "/icon-192.png",
    badge: "/icon-192.png",
    // tag/renotify — чтобы уведомления не копились в шторке Android до
    // бесконечности (новое заменяет предыдущее вместо добавления в стопку).
    // Раньше это было одной из причин, почему значок на иконке приложения
    // продолжал висеть даже после прочтения всех сообщений внутри приложения:
    // многие лаунчеры Android считают значок по количеству активных системных
    // уведомлений в шторке, а не по Badging API.
    tag: "uchet-notification",
    renotify: true,
    data: { url: data.url || "/" },
  };
